...
\section*{Abstract}
\textit{This report presents the work conducted during my M1 internship at the Nakashima Laboratory, focusing on the SWUMANOID humanoid swimming robot project. The previous control architecture severely limited hydrodynamic experiments due to a low control frequency, unstable communications, and mechanical jerks. The objective of my internship was to improve and modernize the command and control system of the robot to overcome these limits. To achieve this, the approach included the assembly of a custom AtomS3R-based main board, the implementation of a deterministic 3\,Mbps RS-485 communication bus, and the development of a web interface for real-time telemetry supervision. Furthermore, the kinematic pipeline has been rebuilt using periodic cubic spline interpolation to mathematically guarantee smooth, jerk-free joint trajectories. This was paired with an inertial processing chain that fuses raw IMU data via a quaternion-based complementary filter, compensates for the sensor's mechanical tilt through rigid frame transformations, and extracts stable Euler angles for real-time 3D spatial tracking. This new architecture allows the operator to communicate directly with the robot and provides real-time access to all critical telemetry and status information during pool experiments. Moreover, experimental validation demonstrated the total elimination of signal interference and mechanical jolts. Finally, the system now achieves a highly stable control rate of approximately 90\,Hz while simultaneously acquiring the full telemetry of its 21 joints via an optimized scheduling model. Ultimately, these hardware and software contributions have established a reliable, fault-tolerant, and highly responsive experimental platform ready for future advanced biomechanical research.
}
...
% ==========================================
\clearpage
\pagestyle{empty} % Désactive toute numérotation sur les pages du sommaire
{\let\thispagestyle\empty \tabledematieres} % Empêche LaTeX de forcer un numéro sur la 1ère page du sommaire

% ==========================================
%             BODY OF THE REPORT
% ==========================================
\clearpage
\setcounter{page}{1} % Le compteur démarre à 1 ici
\pagestyle{plain}    % Affiche UNIQUEMENT le numéro de page en bas au centre

\section{Introduction}

\subsection{The Nakashima Laboratory}

The Nakashima Laboratory, led by Professor Motomu Nakashima, belongs to the \textbf{Institute of Science Tokyo}. This university represents the merger of the \textit{Tokyo Institute of Technology} and the \textit{Tokyo Medical and Dental University} \cite{isct2024merger}. The result is an institution built around "convergence science", bringing together engineering, advanced technology, biology and medicine \cite{isct2024merger}. The Nakashima Laboratory sits right at that crossroads, where fundamental mechanics meets human physiology.

Its focus is biomechanics and sports engineering. The main goal is to use robotics and computation to better understand, simulate and reproduce the complex dynamics of the human body \cite{nakashimalab_website}. Working from mathematical models and detailed simulations, such as the SWUM model, which couples rigid-body dynamics with unsteady fluid forces \cite{nakashima2007swum}, the team analyzes and optimizes human motion in different environments. The work spans several scales, from the macroscopic study of fluid forces to a fine musculo-skeletal analysis of physical effort, as shown in the figure \ref{fig:simulations_labo}. \label{text:simulations_labo}



The research is not limited to water, in fact it reaches into many athletic biomechanical problems for example the body dynamics during wheelchair tennis matches, the study of mass distribution and body management during sleep, and the modeling of effort in redundant human tasks.



To test these models in the real world, the lab designs and runs a wide range of experimental devices, shown in the figure \ref{fig:prototypes_labo}. They range from wearable measurement systems for the upper limbs to biomimetic aquatic robots, and up to a full swimming humanoid robot, the SWUMANOID. This robot grew out of years of \textit{Bio-Robotics} research, and it is the project I joined. \label{text:prototypes_labo}
...
To sidestep all of this, the lab built a complete swimming humanoid robot that acts as a stand-in experimental platform \cite{chung2014swimming} \cite{nakashima2016realization}. The base model was designed by reproducing the body geometry of an elite swimmer captured in a 3D scan \cite{chung2013free} \cite{nakashima2016realization}. Figure \ref{fig:modeles_3d} \label{text:modeles_3d} shows this biomimetic approach, comparing the 3D mesh of a real swimmer with the CAD shell adapted for the SWUMANOID.



When I joined the project, it was in a transition phase, the previous SWUMANOID, which swam perfectly, had suffered irreversible damage and could no longer be used. A brand-new version, starting from scratch, had been launched only a few months earlier. This full rebuild covered the structure, the skeleton, the 3D printing of every part, new motors, and a rewrite of the motion files. Figure \ref{fig:robot_moteurs} \label{text:robot_moteurs} shows this new iteration and the layout of its actuation.



Building the robot at full human size is not practical, so its scale was reduced. The robot is roughly 50\% of the real swimmer's size \cite{chung2013free} \cite{chung2014swimming}, but the arms of the new version were modeled at 60\% of full scale to make place for the new motors. These motors are more powerful and, above all, IP68-certified, so fully waterproof, which removes any risk of short circuits. The robot weighs about 7.3 kg, close to 10\% of an adult swimmer's weight, in line with the earlier models \cite{chung2013free} \cite{nakashima2016realization}.

I joined the SWUMANOID team, a group of three where I work alongside my two supervisors, M. YANADORI and M. NAGATA. When I arrived, the team was finishing tests on new coatings for the 3D-printed parts and before that, the parts filled up with water, which changed the buoyancy and sank the robot. The multi-layer waterproofing process, detailed in the figure \ref{fig:waterproof}, was a decisive step for the integrity of the whole system. We settled on urethane as the coating for the PLA+ printed parts which solved the water infiltration problem, and the robot could finally float the moment I arrived, which had not been the case before. \label{text:waterproof}

Within our trio, the work is clearly split. My two supervisors focus on the theoretical and analytical study of motion, their research relies on the simulation software \textit{SWUMSUIT}. M. NAGATA works on steering the robot during swimming, with a particular focus on the dynamics of turning strokes. M. YANADORI studies the torques generated by the robot's joints while swimming, using the current drawn by the motors. These studies advance the lab's research into how muscular limits influence swimming speed and propulsive efficiency~\cite{nakashima2012optimizing}.

The SWUMSUIT interface can extract swimming data such as swimming speed, propulsive efficiency, and joint torques \cite{swumsuit2005}. Built to implement the SWUM (Swimming Human Model) simulation model, it computes the absolute motion of the human body in water by solving the equations of motion of a rigid body, and estimates the fluid forces acting on the body from the input geometry and joint motions \cite{swumsuit2005}. The numerical results let us visualize the swimming dynamics and read the vector distribution of the propulsive forces, as the captures in the figure \ref{fig:swumsuit_sim} show. \label{text:swumsuit_sim}
...
To conclude this introduction, the remainder of this report details the methodology, hardware design, and software implementation required to upgrade the SWUMANOID's control architecture. The \textbf{Section~\ref{sec:web_interface_sec}} presents the development of the new embedded firmware and the user web interface. It focuses on the migration to the AtomS3R microcontroller, network stability improvements, and the design of an optimized, non-blocking telemetry logging system. 

\textbf{Section~\ref{sec:motors_board_configuration}} focuses on the hardware developments and experimental validations. It details the design of the custom main board, the resolution of RS-485 communication limitations, and the physical testing on the robot's actuators to secure a high-frequency control loop. 

The report concludes with a summary of the outcomes and the platform's readiness for future hydrodynamic research. Finally, the \textbf{Annexes} provide the in-depth mathematical and theoretical frameworks supporting this engineering work. They detail the periodic cubic spline interpolation algorithm developed to guarantee smooth, jerk-free joint trajectories, as well as the IMU signal processing chain, with the quaternion-based complementary filter and rigid frame transformations required to track the robot's 3D spatial orientation.
...
The original firmware had been written and validated for the \textbf{M5Stack AtomS3} development board \cite{m5stack_atoms3}. But to meet the new requirements, the hardware had just been upgraded to the \textbf{M5Stack AtomS3R} \cite{m5stack_atoms3r} right before I arrived, we can see the two microcontrollers in the figure \ref{fig:m5stack_comparison}. \label{text:m5stack_comparison}
...
I also added the ability to upload motion files straight from SWAM (.dat), converted on the fly during upload, to make the web control interface fully standalone, smooth and secure. The trajectory conversion logic runs entirely on the client (JavaScript) and I ported it from the MATLAB code used previously and provided by M. Yanadori. More precisely, when parsing the \texttt{.dat} file in JavaScript, each joint angle is converted to Dynamixel ticks using the affine formula of the X-series motors~\cite{dynamixel_x_manual}:
\begin{equation}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.46\textwidth}
        ...
        \caption{Network \& Scan panel in the dashboard.}
...
When the components arrived, I assembled the new control board with the three active elements showing in the figure~\ref{fig:composants}, the AtomS3R microcontroller~\cite{m5stack_atoms3r}, the RS-485 transceiver module with an accessible direction pin, and the DC-DC step-down converter supplying the logic power. \label{text:composants}

Above all, this new architecture made it possible, for the first time, to test the \emph{real} motors of the robot. In fact, the SWUMANOID is driven by three families of XW-series Dynamixel servomotors (waterproof, IP68 rating), spread across the body according to their torque needs presented in the figure~\ref{fig:swumanoid_map}: XW540-T140-R and XW540-T260-R~\cite{dynamixel_xw540t140,dynamixel_xw540t260} for the most heavily loaded joints, and XW430-T200-R~\cite{dynamixel_xw430t200} for the distal joints, all these motors are waterproof. \label{text:swumanoid_map}



So, to validate the hardware architecture under representative conditions, I built a test circuit combining the new components with a cluster of five \emph{real} Dynamixel motors from the robot with two XW430-T200-R, two XW540-T260-R and one XW540-T140-R. Each was given the ID matching its actual function in the SWUMANOID, so the bench faithfully reproduces a subset of the robot. The XW540-T140-R thus corresponds to a \textbf{shoulder joint}, whose behavior gets a detailed analysis in the results section. Unlike the first bench, limited to substitute motors, this configuration uses far more powerful and extremely responsive, they allow much better command tracking.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{schemas/schema_5_moteurs_haut_baudrate.png}
...
\hline
\textbf{Control Parameter} & \textbf{Value} & \textbf{Telemetry Parameter} & \textbf{Value} \\ \hline
Command rate & 100\,Hz & Logged motors & 21 (all) \\
Commanded cycle time & 1.80\,s & Sampling interval & 6 frames \\
Real control rate & $\approx 90$\,Hz & Motors polled per cycle & 11 \\ \hline
\end{tabular}
\caption{Optimal operating parameters retained for swimming experiments.}
...
The primary objective of this internship was to overhaul the SWUMANOID's control system, transforming it from a bottlenecked, unstable architecture into a highly responsive platform capable of supporting cutting-edge hydrodynamic research. To address the initial problematic characterized by a locked 72 Hz frequency, Wi-Fi packet loss, and severe mechanical jerks, a complete approach was required, connecting hardware design, embedded software, and network architecture.

By designing a custom main board utilizing the AtomS3R and a MAX3485 transceiver with explicit Driver Enable control, I succeeded in achieving a 3 Mbps baud rate. Coupled with a new web architecture using Server-Sent Events and binary PSRAM logging, the system is now completely immune to the Wi-Fi dropouts and SD-card latencies that previously plagued the pool experiments.

The most significant outcome of this work is the experimental characterization and optimization of the RS-485 bus time budget. Reaching high frequencies exposed a critical trade-off between motion smoothness and telemetry acquisition. Through rigorous testing, it was proven that polling all 21 motors sequentially at maximum speed saturates the half-duplex bus, degrading the control loop. However, by intelligently spacing the read operations and pre-compensating the cycle time, an optimal operating point was established. The robot now sustains a highly stable real control rate of approximately 90 Hz while flawlessly logging the full body state.

This internship shows that the software ambitions are intrinsically bound by hardware physics. Moreover, the delay in acquiring the high-speed components forced a temporary reliance on a low-frequency setup, which exposed the necessity of a robust periodic cubic spline interpolation which demonstrated that hardware constraints can drive software innovation, resulting in a system that is now fault-tolerant at both levels. Navigating the entire robotics stack to upgrade the SWUMANOID, from soldering a custom AtomS3R PCB tailored to fit inside its waterproof head, to coding a standalone poolside web interface and formulating a deterministic RS-485 scheduler for the Dynamixel motors, anchored theoretical engineering within the tangible constraints of a biomechanical research platform.

With stable, continuous, and fully monitored swimming cycles now guaranteed, the experimental platform is fully operational for the SWUMANOID. The Nakashima Laboratory can now confidently focus on its primary goal, extracting and analyzing the unsteady fluid forces of human swimming to push the boundaries of biomechanics and sports engineering.
...
\bibitem{isct2024merger} INSTITUTE OF SCIENCE TOKYO. "Establishment of Institute of Science Tokyo: Merging Tokyo Tech and TMDU." Accessed on June 5, 2026. URL: \url{https://www.isct.ac.jp/en/001/about}.
...
\textit{\textbf{Note:} To facilitate navigation, the figures are interactive, you can click on the \textbf{[$\uparrow$]} button at the end of each caption to automatically jump back to the exact paragraph where the figure was discussed.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Simulations used in the laboratory to study body dynamics. \hyperref[text:simulations_labo]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Physical experimental platforms developed by the laboratory \cite{nakashimalab_website}. \hyperref[text:prototypes_labo]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=5cm]{Introduction/Model_3D_real_swimmer.png}
        \caption{3D scan of the swimmer}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=5cm]{Introduction/SWUMANOID_Model_3D.png}
        \caption{3D model adapted for the robot}
    \end{subfigure}
    \caption{Robot geometry derived from the morphology of an elite swimmer. \hyperref[text:modeles_3d]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=6cm]{Introduction/SWUMANOID_Front_Picture.png}
        \caption{New version of the SWUMANOID}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=6cm]{Introduction/SWUMANOID_Motors_Ids.png}
        \caption{Motor map and IDs}
    \end{subfigure}
    \caption{Hardware and kinematic overview of the robot. \hyperref[text:robot_moteurs]{\textbf{[$\uparrow$]}}}
    \label{fig:robot_moteurs}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Introduction/Werproof_method.png}
    \caption{Waterproofing protocol for the PLA+ parts. \hyperref[text:waterproof]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Interface and renders of the \textit{SWUMSUIT} simulation software. \hyperref[text:swumsuit_sim]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \caption{Suspended test rig in the laboratory. \hyperref[text:air_test]{\textbf{[$\uparrow$]}}}
    \label{fig:air_test}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Free-swimming pool experiment with the SWUMANOID. \hyperref[text:pool_experiment]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \caption{Layout of the experiment recording setup. \hyperref[text:dispositif_experiment]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Visual and technical comparison of the two microcontrollers. \hyperref[text:m5stack_comparison]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.25\textwidth}
        ...
        \label{fig:comp_buck}
    \end{subfigure}
    \caption{Components of the new \textit{main board}. \hyperref[text:composants]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.34\textwidth}
        \centering
        \includegraphics[height=13cm]{schemas/swumanoid_motor_map.png}
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \label{fig:motor_xm}
    \end{subfigure}
    \caption{SWUMANOID motor configuration. \hyperref[text:swumanoid_map]{\textbf{[$\uparrow$]}}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \label{fig:comp_pcb}
    \end{subfigure}
    \caption{The two smaller parts chosen to shrink the board down to size. \hyperref[text:new_components]{\textbf{[$\uparrow$]}}}
...
\subsection{Motion Kinematics and Interpolation}
\label{subsec:method_interpolation}

\subsubsection{The Problem of Previous System}
...
\subsubsection{Choosing the Cubic Spline}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{graphiques/202607141616_ID_4_Spline_Cubic_Pure_90Hz.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{graphiques/202607141612_ID_4_Spline_Cubic_Pure_50Hz.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{graphiques/202607141611_ID_4_Spline_Cubic_Pure_36Hz.png}
...
\subsubsection{Solving the Cyclic System}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graphiques/202607141717_ID_4_Spline_Cubique_Periodique_36Hz.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graphiques/202607141720_ID_4_Spline_Cubique_50Hz.png}
...
\subsubsection{Minimum-Jerk Ramp}
...
\subsection{Angular Configuration of the SWUMANOID via the IMU}
\label{subsubsec:calcul_angle}
...
\subsubsection{Acquisition and Fusion of the Raw Data}
\label{subsubsec:acquisition}
...
\subsubsection{Frame Change and Calibration}
\label{subsubsec:calib}
\label{subsubsec:tare}
...
\subsubsection{Euler Angle Extraction and Low-Pass Filtering}
\label{subsubsec:extract_filter}
