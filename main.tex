...
So the fix is not to look for a "faster" chip, since the SP3485 is already enough. It is to use a board where the DE direction pin is driven \emph{explicitly} by the microcontroller, instead of being left to a too-slow automaton. So I drew up the list of parts to order to replace the \textit{Atomic RS485} base:
\begin{itemize}
...
\paragraph{Optimal parameters retained :} This campaign on the real robot fully confirms the budget model and leads to a parameter set determined end to end by experiment, which will serve as the reference for the future swimming experiments:
\begin{itemize}
...
The $N$ frames of the source file are indexed on a uniform grid $x_i = i$, $i = 0,\dots,N-1$ (step $h = x_{i+1} - x_i = 1$), and we write $y_i$ for the angular command of a joint at frame $i$ (each motor is handled independently by the same algorithm). On each interval $[x_i,\, x_{i+1}]$, the trajectory is modeled by a cubic polynomial
