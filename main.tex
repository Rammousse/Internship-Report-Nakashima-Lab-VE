...
Each of the two tridiagonal systems $Tz = d$ and $Tw = u$ is solved by the Thomas algorithm (or Tridiagonal Matrix Algorithm, TDMA)~\cite{burden_faires}, a variant of Gaussian elimination specialized for tridiagonal matrices, of complexity $\mathcal{O}(N)$ (instead of $\mathcal{O}(N^3)$ for a general inversion). 

For a generic system $T x = r$ with diagonal $T_{i,i}$ and unit sub/super-diagonals, the algorithm runs in two passes:
...
\begin{thebibliography}{99}
\setcounter{enumiv}{13} % Resume numbering at [11]
