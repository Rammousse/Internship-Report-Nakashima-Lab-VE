...
Despite this topology, the bus experienced an $80\%$ packet loss, causing severe motion instability. The problem could be a signal reflection in an unterminated differential bus. To achieve impedance matching and absorb these reflections, 120,$\Omega$ termination resistors~\cite{dynamixel_x_manual} were installed at both ends of the transmission line (at the AtomS3R base and the terminal motor). This hardware correction successfully eliminated signal interference, reducing packet loss to $0\%$ and enabling stable, synchronized motor control at 80\,Hz.
...
On the software side, the PlatformIO bus driver was updated to leverage the hardware RS-485 half-duplex mode of the ESP32-S3's UART~\cite{esp32s3_uart}. By linking the transceiver's DE pin to the UART controller's RTS pin, the transmit/receive direction is toggled automatically in direct synchronization with the last transmitted stop bit. This eliminates software latency and frees the bus in under a microsecond, reliably supporting baud rates of 1 Mbps and up to 3 Mbps. This explicit direction control allows the motors' \textit{Return Delay Time} to be minimized, securing the high-frequency control loop and laying the groundwork for the final main board architecture.


\subsection{New Main Board and Configuration}

When the components arrived, I assembled the new control board. Its three active elements showing in the figure~\ref{fig:composants} are the AtomS3R microcontroller~\cite{m5stack_atoms3r}, the RS-485 transceiver module with an accessible direction pin, and the DC-DC step-down converter supplying the logic power.
...
Above all, this new architecture made it possible, for the first time, to test the \emph{real} motors of the robot. In fact, the SWUMANOID is driven by three families of XW-series Dynamixel servomotors (waterproof, IP68 rating), spread across the body according to their torque needs (Figure~\ref{fig:swumanoid_map}): XW540-T140-R and XW540-T260-R~\cite{dynamixel_xw540t140,dynamixel_xw540t260} for the most heavily loaded joints, and XW430-T200-R~\cite{dynamixel_xw430t200} for the distal joints, all these motors are waterproof.
...
Within this circuit showed in the figure~\ref{fig:circuit_test}, the ESP32-S3 architecture allows flexible pin routing. I configured the communication lines as follows, in line with the embedded bus driver:
\begin{itemize}
...
Then, we moved the whole circuit off the breadboard and onto that PCB. The AtomS3R, the MAX3485, the little converter and the termination resistors were all soldered down and wired point to point, so nothing can pop loose when the lid is closed. The board came out small, flat and sturdy. It drops straight into the waterproof case, which in turn slots into the head we can see in figure~\ref{fig:final_board}.
